 ...
 * NOTE: This function exists for compatibility but is no longer used in sync
 * Each page keeps its own HEAD section to preserve unique meta tags and stylesheets
 */
 ...
 * Determines page type from filepath and adds active class
 */
function markActiveNav(header, filePath) {
  // Map file paths to their primary nav section
  const pageMap = {
  ...
  // Determine current page section from filepath
  let currentHref = null;
  for (const [pathSegment, href] of Object.entries(pageMap)) {
    // Check if filepath contains this path segment (handles both "services/certificates.html" and "service-details/...")
    if (filePath.startsWith(pathSegment) || filePath.includes(`/${pathSegment}/`) || filePath.includes(`/${pathSegment}.html`)) {
      currentHref = href;
      break;
    }
  }
  ...
  if (!currentHref) {
    return header; // Home page or unknown section
  }
  
  // Find and mark the current nav link as active
  // Match: <a href="/government/">...any content...</a>
  const pattern = new RegExp(
    `<a([^>]*?)href="${currentHref.replace(/\//g, '\\/')}"([^>]*)>`,
    'g'
  );
  
  let fixed = header.replace(pattern, (match, before, after) => {
    // Check if class attribute already exists
    if (after.includes('class=')) {
      // Add active to existing class
      return `<a${before}href="${currentHref}"${after.replace(/class="([^"]*)"/, 'class="$1 active')}>`; 
    } else {
      // Add new class attribute
      return `<a${before}href="${currentHref}" class="active"${after}>`;
    }
  });
...
/**
 * Rebuild HTML with new header/footer but PRESERVE the original head section
 */
function rebuildHtml(originalHead, header, mainContent, footer, filePath) {
  // Fix paths based on file's location
  const headerFixed = fixHeaderFooterPaths(header, filePath);
  const headerWithActive = markActiveNav(headerFixed, filePath);
...
function main() {
  try {
    ...
    // NOTE: Head section is no longer synced - each page preserves its own HEAD
    // This ensures page-specific stylesheets and meta tags are maintained
    ...
    for (const file of htmlFiles) {
      try {
        ...
        // Extract the file's ORIGINAL head to preserve it
        const originalHead = extractHead(fileContent);
        
        // Extract main content from existing file
        const mainContent = extractMainContent(fileContent);
        
        // Rebuild the file with preserved head
        const newContent = rebuildHtml(originalHead, header, mainContent, footer, file);
